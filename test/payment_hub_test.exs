defmodule PaymentHubTest do
  ...
  describe "withdraw" do
    ...
    test "transfer 1 DML when signature is valid", %{accounts: accounts, hub_address: hub_address} do
      account = Enum.at(accounts, 1)

      assert BasicToken.balance(hub_address) == 10
      assert BasicToken.balance(account) == 0
      assert {:ok, _} = PaymentHub.withdraw(account, 1, 1, "signature")
    ...
    test "transfer 5 DML (-fee) when signature is valid", %{owner: owner, accounts: accounts, hub_address: hub_address} do
      ...
      assert {:ok, _} = PaymentHub.withdraw(account, 5, 1, "signature")
      assert BasicToken.balance(hub_address) == 6
      assert BasicToken.balance(account) == 4
    end

    @tag :skip
    test "prevent transfer when signature is invalid"

    @tag :skip
    test "prevent transfer when signature is valid, but created by non-owner"
    ...
    test "prevent transfer when address is blacklisted", %{owner: owner, accounts: accounts, hub_address: hub_address} do
      ...
      response = PaymentHub.withdraw(account, 1, 1, "signature")
    ...
    test "prevent transfer when nonce was already used", %{accounts: accounts, hub_address: hub_address} do
      account = Enum.at(accounts, 1)

      # Transfer to account, creating nonce
      assert {:ok, _} = PaymentHub.withdraw(account, 1, 2, "signature")

      # Try to transfer to account again, with same nonce
      response = PaymentHub.withdraw(account, 1, 2, "signature")
